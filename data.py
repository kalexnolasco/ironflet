...
PHASES: list[Phase] = [
    Phase(1, "1-3", "Conditioning", "Hypertrophy", "#00D4AA",
          "3×12", "1' sets / 2-3' exercises",
          "No absolute failure. 20' continuous cardio post-weights. Abs 2 days/week.",
          "high"),
    Phase(2, "4-6", "Total Hypertrophy", "Muscle Failure", "#FF6B35",
          "3×8-10", "1'30\" sets (max 2')",
          "Failure on last set. Same exercises. 20' cardio. More intense abs.",
          "high"),
    Phase(3, "7-9", "Functional Hypertrophy", "Strength + Hypertrophy", "#7C5CFC",
          "4×5-6 (*=3×8)", "2-2'30\" / 3' ex. / 1' on *",
          "Increase loads. *= 3×8 to failure. 20' cardio. Abs 5×8-12.",
          "high"),
    Phase(4, "10", "Deload", "Recovery", "#FFD93D",
          "3-4×12", "1' sets / 2' exercises",
          "Low intensity, no failure. 3 days full body.",
          "low"),
    Phase(5, "11-14", "Weider Hypertrophy", "High Volume", "#FF3366",
          "Ascending Pyramid", "1'30\" sets / 3' exercises",
          "5 days, 1 muscle/day. Failure on all sets except first.",
          "low"),
    Phase(6, "15-18", "High Intensity", "Drop Sets + Compound", "#FF8A5C",
          "Drop sets (e.g.: 10,8,6+12↓)", "1' sets / 3' exercises",
          "Drop sets on last set. 5' treadmill pre-weights.",
          "low"),
    Phase(7, "19-24", "Fat Loss", "Bi-sets / Supersets", "#4ECDC4",
          "BI 4×8+10 / SS 4×10+10", "Min. between blocks, 1-1'30\"",
...
DAYS_PHASE_1_3: list[TrainingDay] = [
    TrainingDay("Chest - Biceps", [
        "Smith Bench Press", "Incline DB Flyes",
        "Incline DB Press", "Pec Deck *",
        "Straight Bar Curl", "EZ Bar Preacher Curl",
        "Alternate Hammer Curl *",
    ]),
    TrainingDay("Back - Triceps", [
        "Front Lat Pulldown", "DB Row",
        "Behind-Neck Lat Pulldown", "Seated Cable Row *",
        "EZ Bar Skull Crushers", "Smith Close-Grip Press",
        "Rope Pushdown *",
    ]),
    TrainingDay("Shoulder - Leg", [
        "Seated DB Press", "Arnold Press *",
        "Lateral Raises", "Standing Rear Delts",
        "Leg Extensions *", "Leg Press",
        "Romanian Deadlift", "Lying Leg Curl *",
        "Lunges", "Standing Calf Raise", "Seated Calf Raise *",
    ]),
]

DAYS_PHASE_4: list[TrainingDay] = [
    TrainingDay("Day 1 Full Body", [
        "Barbell Bench Press", "Front Lat Pulldown",
        "DB Shoulder Press", "Barbell Curl",
        "Triceps Pushdown", "Squat",
    ]),
    TrainingDay("Day 2 Full Body", [
        "Incline DB Press", "Barbell Row",
        "Arnold Press", "Alternate DB Curl",
        "Rope Pushdown", "Romanian Deadlift",
    ]),
    TrainingDay("Day 3 Full Body", [
        "Machine Flyes", "Seated Cable Row",
        "Lateral Raises", "Hammer Curl",
        "Skull Crushers", "DB Lunges",
    ]),
]

DAYS_PHASE_5: list[TrainingDay] = [
    TrainingDay("D1: Chest + Abs", [
...
DAYS_PHASE_7: list[TrainingDay] = [
    ...
    TrainingDay("W4-6: Chest-Back (SS)", [
    ...
    TrainingDay("W4-6: Shoulder-Leg (SS)", [
    ...
    TrainingDay("W4-6: Arms (Antag. SS)", [
...
UL_PHASES: list[Phase] = [
    Phase(1, "1-6", "Foundation", "Build base", "#00D4AA",
          "4×6-10", "1'30\"-2' sets",
          "Same loads. RIR 2-3. Add weight when top reps are easy.",
          "high"),
    Phase(2, "7-12", "Hypertrophy Block", "Volume + Intensity", "#7C5CFC",
          "4×8-12", "1'-1'30\" sets",
...
UL_DAYS_FOUNDATION: list[TrainingDay] = [
    TrainingDay("Upper A: Push focus", [
        "Barbell Bench Press", "Barbell Row",
        "Seated DB Press", "Pull-ups",
        "EZ Bar Skull Crushers", "Barbell Curl",
    ]),
    TrainingDay("Lower A: Squat focus", [
        "Squat", "Romanian Deadlift",
        "Leg Press", "Lying Leg Curl",
        "Standing Calf Raise", "Plank",
    ]),
    TrainingDay("Upper B: Pull focus", [
        "Incline DB Press", "Front Lat Pulldown",
        "Lateral Raises", "DB Row",
        "Triceps Pushdown", "Hammer Curl",
    ]),
    TrainingDay("Lower B: Deadlift focus", [
        "Deadlift", "Leg Press",
...
WF_DAYS_SHAPE: list[TrainingDay] = [
    TrainingDay("Lower A: Glute focus", [
        "Squat [4×12]",
    ...
    TrainingDay("Lower B: Quad focus", [
        "Barbell Lunges [4×12]",
        "Leg Extensions [3×15]",
...
WH_PHASES: list[Phase] = [
    Phase(1, "1-8", "Home Foundation", "Bodyweight + DBs", "#FF3366",
          "3×10-15", "45s-1' sets",
...
WS_PHASES: list[Phase] = [
    Phase(1, "1-6", "Technique Base", "Submax 5×5", "#00D4AA",
          "5×5 @ RPE 6-7", "2-3' sets",
          "Submaximal loads. Add weight when 3 sessions at the same load feel easy.",
          "high"),
    Phase(2, "7-12", "Strength Block", "Heavy 5×3-5", "#FF6B35",
          "5×3-5 @ RPE 8", "3-5' sets",
          "Top set RPE 8. Back-off sets 10-15% lighter for volume.",
          "high"),
]

WS_DAYS_BASE: list[TrainingDay] = [
    TrainingDay("Squat Day", [
        "Squat",
        "Romanian Deadlift",
        "Leg Extensions",
        "Lying Leg Curl",
...
PPL_PHASES: list[Phase] = [
    Phase(1, "1-6", "Foundation", "Build base", "#00D4AA",
          ...
          "high"),
    Phase(2, "7-12", "Intensification", "Volume + Intensity", "#FF6B35",
          "4×8-12", "1'-1'30\" sets",
...
WEEK_ROTATION: list[list[str]] = [
    ["Chest-Biceps", "Back-Triceps + Abs", "Rest",
     "Shoulder-Leg", "Chest-Biceps + Abs"],
    ["Back-Triceps", "Shoulder-Leg + Abs", "Rest",
     "Chest-Biceps", "Back-Triceps + Abs"],
    ["Shoulder-Leg", "Chest-Biceps + Abs", "Rest",
     "Back-Triceps", "Shoulder-Leg + Abs"],
]
...
ROUTINES: list[Routine] = [
    Routine(
        ...
        phases=PHASES,
        days_by_phase={
            1: DAYS_PHASE_1_3, 2: DAYS_PHASE_1_3, 3: DAYS_PHASE_1_3,
            4: DAYS_PHASE_4, 5: DAYS_PHASE_5,
            6: DAYS_PHASE_6, 7: DAYS_PHASE_7,
        },
...
EXERCISE_CATALOG: dict[str, list[str]] = {
    "Chest": [
        "Barbell Bench Press", "Smith Bench Press",
        "Incline DB Press", "Incline Machine Press",
        "Decline Press", "Close-Grip Bench Press",
        "Incline DB Flyes", "Pec Deck",
        "Cable Crossover", "Push-ups",
    ],
    "Back": [
        "Front Lat Pulldown", "Behind-Neck Lat Pulldown",
        "V-Grip Lat Pulldown", "Pull-ups",
        "Barbell Row", "DB Row",
        "Seated Cable Row", "T-Bar Row",
        "Machine Row", "Deadlift", "Romanian Deadlift",
    ],
    "Shoulder": [
        "Barbell Military Press", "Seated DB Press",
        "Arnold Press", "Lateral Raises",
        "Machine Lateral Raises", "Plate Front Raises",
        "DB Rear Delts", "Cable Rear Delts", "Upright Row",
    ],
    "Biceps": [
        "Straight Bar Curl", "EZ Bar Curl",
        "EZ Bar Preacher Curl", "Alternate DB Curl",
        "Simultaneous DB Curl", "Hammer Curl",
        "Cable Curl", "Incline DB Curl", "Reverse Curl",
    ],
    "Triceps": [
        "EZ Bar Skull Crushers", "Close-Grip Bench Press",
        "Rope Pushdown", "Triceps Pushdown",
        "Reverse Pushdown", "Parallel Bar Dips",
        "Overhead Extension", "Triceps Kickback",
    ],
    "Leg": [
        "Squat", "Leg Press",
        "Leg Extensions", "Lying Leg Curl",
        "Seated Leg Curl", "Lunges",
        "Standing Calf Raise", "Seated Calf Raise", "Calf Press",
    ],
    "Abs": [
        "Cable Crunch", "Hanging Abs",
        "Floor Crunch", "Floor Obliques",
        "Plank", "Leg Raises", "Hyperextensions",
    ],
    "Cardio": [
        "Treadmill", "Bike",
